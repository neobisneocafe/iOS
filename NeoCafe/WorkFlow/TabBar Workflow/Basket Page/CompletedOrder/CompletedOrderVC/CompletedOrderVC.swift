...
class CompletedOrderVC: BaseViewController {
    ...
    private let viewModel = CompletedOrderVM()
    
    private var items: [CompletedOrderTableViewCell.Props] = []
    ...
    override func viewDidLoad() {
        ...
        viewModel.fetchOrdersCompleted(id: id) { [weak self] response in
            guard let self, let branch = response.branch else { return }

            items = response.dishes?.map {
                CompletedOrderTableViewCell.Props(
